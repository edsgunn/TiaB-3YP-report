...
Since this collapsing of dimensions has already been conducted in \ref{Chapter3} we are able to take full advantages of NLP techniques.
...
However, we felt \textbf{NEED STRONGER PROOF} 
...
% MySong, the first attempt focusing specifically on a vocal melody in \cite{MySong} used an augmented hidden markov model with parameters such that users could adjust the "Happy Factor" and "Jazz Factor" to alter the mood of the generated chords to their preference.
% A weakness of the hidden markov model is that at each timestep only the previous timestep is considered when suggesting a chord. 
...
% \cite{ReinforcementLearning} proposed a model  that learns a structured representation for use in symbolic melody harmonization. It utilises two layers of LSTMs to detemine when each chord should be played then utilises a policy gradient RL method to select each chord.
% MuseGAN \\      
% ChordGAN \\       
...
By modelling the $\boldsymbol{X}$ and $\boldsymbol{Y}$ Markov Processes \textbf{references?} 
...
The HMM is trained on a reduced version of the \textbf{Include link?} 
...
\textbf{Benefits of mask with transformer in being able to specify given chords using mask}
\section{Our Model}
...
The standard tool to find the gradients is the backpropagation algorithm \textbf{REFERENCE BACKPROP} 
...
An example of a confusion matrix is shown in \ref{fig:confusion_ideal}
...
A confusion matrix of these results can be seen in figure \ref{fig:confusion_real}
\begin{figure}
    \centering
