...
\\Hybrid recommendation systems have two predominant designs, parallel and sequential. A similar design provides the input to multiple recommendation systems, and each of those recommendations is combined to generate one output. 
...
Without up-to-date metrics (user engagement, ratings, etc.), it isn't easy to retrain and provide new recommendations with updated items and ratings from various users.
...
CommonStandard statistical accuracy measures to evaluate the accuracy of a recommender are RMSD, MAE, and k fold cross-validation.
...
\begin{itemize}
\item Imagine you’ve built a model that will predict how well a user will rate an item based on features. K fold cross-validation can be used to infer the results of the model through accuracy metrics
\item Same idea as a train test split, except we create K many randomly assigned training and test sets
\item Each training set/fold is used to train on the recommendation system independently and then measure the accuracy of the resulting systems against the test set
\item We take the average accuracy score to see how well the recommendation system is learning
...
\begin{itemize}
\item A similar metric to MAE but has a stronger penalty for when the prediction is very far from the true value and a weaker penalty for when the prediction is closer to the true value
