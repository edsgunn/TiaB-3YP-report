...
\begin{figure}[h]
	...
	\begin{tikzpicture}[>=latex']
		...
		%beige bg
		\node (bg) [right= 4.5cm of n1, anchor=center, draw=none, fill={rgb:orange,1;yellow,2;pink,5}, minimum width=8cm,minimum height=2.5cm]{};
		...
		\node (n6) [block, draw=red, right=1cm of n5]
					...
					(Notes and keys)};
		\node [coordinate, below right =1cm and 1cm of n3] (right) {};  %% Coordinate on right and middle
		\node [coordinate, above left =0.8cm and 1cm of n4] (left) {};  %% Coordinate on left and middle
...
\begin{enumerate}
	...
	$CHANGE$
	CWT finds how alike a wavelet is in a signal, given the dilation and translation parameter of the wavelet. \cite{wavelet}
	...
	\begin{figure}
		...
		\begin{subfigure}{.3\textwidth}
		  \centering
		  \includegraphics[width=1\linewidth]{haar.png}
		  \caption{Haar wavelet}
		  \label{Haar}
		\end{subfigure}%
		\begin{subfigure}{.3\textwidth}
			\centering
			\includegraphics[width=1\linewidth]{order2gaussian.png}
...
According to (D SHUKLA, 2003), there are 2 major concerns with using wavelet transform. $PROVE PLEASE$ 
...
The implementation of spectral noise gating can be 
summarised according to \cite{spectralflowchart} as shown in \autoref{spectralflowchart}
\begin{figure}
	\centering
...
for choosing LPF over a band-pass filter is that ambient noise is ususally low frequency and
$INCLUDE WHY NOT BPF$

spectral reduction is effective 
...
\begin{enumerate}
	...
	\[w(n) = \frac{1+cos(2\pi n/N-1)}{2}, \text{for } 0<= n<= N-1\] where $N$ is the number of samples.\\
	...
	\begin{figure}
		\centering
		\includegraphics[width=0.5\textwidth]{CREPE.png}
...
\begin{figure}[h]
	\centering
	\includegraphics[width=0.5\textwidth]{CREPEperf.png}
...
On top of the convolutional neural network model, we can integrate Bayesian statistics to 
improve the model.
...
\label{sec:KDA}
Other than detection the pitch/ note of the input audio, it is also necessary to estimate the key of the music so as to help with the identification
of chords with the model in \textbf{INSERT}
...
%There are mainly 2 types of models explored currently, which are template-based and Hidden Markov Model(HMM)-based approaches. We will focus on the former approach since 
%it has a higher recognition rate than the HMM-based approach.

Krumhansl-Schmuckler algorithm (template-based) \cite{template} is an experimentally measured tonal hierarchy is introduced by Krumhansl and Kessler (1982), in which it contains 24 tonal profiles for major and minor keys in total.
...
Assuming we are using the template from Krumhansl and Kessler for now \cite{templatedata} and it is represented as 24 vectors $\vec{P_{i,k}} \in \mathbb{R}^{12 \times 1}$, 
