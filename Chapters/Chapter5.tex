...
\begin{enumerate}[label=\textbf{\arabic*})]
    ...
    \begin{enumerate}
        ...
        \item High frequencies and low frequencies components have the same time resolution. As frequency increases, the rate of 
        change of the signal increases, thus high-frequency signals contain more information in a window than that of 
        low frequency. Therefore =a higher time resolution is needed for high-frequency signals.
    \end{enumerate}
    ...
    where $x(t)$ is the original signal, $\psi(t)$ is the mother wavelet, $a$ is a dilation parameter and $b$ is a translation parameter \footnote{wavelet_denoise}.
    ...
    where $n$ is delay parameter, $N$ is the length of signal, $\psi$ is the discretized mother wavelet\footnote{wavelet_denoise}. 
    ...
    Assuming $d(n)$ and $x(n)$ have no correlation, and we perform a short-time fourier transform (STFT) on \cref{sreduction}:
...
do not feed a suitable mother wavelet, the performance is unsatisfactory that the model cannot distinguish between desired and undesired 
signals while decreasing $P_{signal}$ at the same time. This is unfavourable when it comes to improving the SNR.\\
...
\begin{equation}
    \abs{\hat{X}(\omega,k)} = \begin{cases}
        \abs{\hat{X}(\omega,k)}   & \text{if} \hat{X}(\omega,k) \geq 0\\
        0                     & \text{otherwise.}
    \end{cases}
\end{equation}
...
After referring to the measurements taken from 192 participants\footnote{f0age} and the pitch range
set by Praat\footnote{praat} (a software for speech analysis), a simple quadratic
modelling of $f_0$ according to age and gender is as below:
\[f_{0,male}(n) = 0.07n^2 - 7.5n + 280, \text{ for } 4 \leq n\leq 93 \label{male} \] 
\[f_{0,female}(n) = 0.02n^2 - 3n + 287, \text{ for } 4 \leq n \leq 93 \label{female} \] 
where $n$ is the age of the user.

However, this is just the mean \[f_0\] thus we are going to only extract the varying part 
(i.e. part without the constant)
%WHAT IS THIS%
...
\begin{enumerate}[label=\textbf{\arabic*})]
    ...
    \begin{figure}[h]
        ...
        \caption{An example signal with zero-crossings marked in dotted lines \footnote{zcr} }
    \end{figure}
    ...
    Exploiting this fact, HPS algorithm creates multiple downsampled signal spectrums and compares them with the original spectrum as shown in 
    ...
    \begin{figure}[h]
        ...
        \caption{Harmonically compressed log spectra \footnote{HPS} }
        \label{HPS}
    \end{figure}
    ...
    window size. Note that as $\tau$ increases, $W$ decreases and the envelope of the function decreases, as shown in  
    \cref{taped}.
    ...
    \begin{figure}
        ...
        \begin{subfigure}{.3\textwidth}
          ...
          \caption{$r_t(\tau)$ calculated with \\ \cref{signal}}
          \label{taped}
        \end{subfigure}%
        \label{YIN}
    \end{figure}
    ...
    It consists of a deep convolutional neural network (CNN) trained by synthesized audio from the RWC Music Database \footnote{rwcdb}
    and MedleyDB \footnote{medleydb}. 
    ...
    As shown in \cref{CREPE}, there are 6 hidden layers and each layer is followed by a dropout layer with a dropout probability of 0.25. 
    ...
    filters, so kernel size refers to the size of the filter mask and the number of kernels relates to the number of output features desired. \cref{CREPE} shows the hyperparameters
    used in CREPE.
    ...
    \begin{figure}
        ...
        \caption{The architecture of the CREPE algorithm\footnote{CREPE}}
        \label{CREPE}
    \end{figure}
...
In exchange, frequency domain models asre less sensitive to noise and perform well on polyphonic singings.
...
\begin{figure}[h]
    ...
    \caption{Raw Pitch Accuracy and Raw Chroma Accuracy with the standard deviations for the 3 PDA tested by Kim et al. \footnote{CREPE}}
    \label{CREPEperf}
\end{figure}
...
\begin{equation}
    T_{i-1} = T_{i-1} + \frac{T_i}{2}  \quad\text{and}\quad   T_{i+1} = T_{i+1} + \frac{T_i}{2}
\end{equation}
...
The prior $P(A)$ can be obtained from the demographic characteristics collected, i.e. age and gender, with the afore modelled 
\cref{male} and \cref{female}.
