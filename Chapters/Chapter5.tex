...
\begin{assumption}-\label{as:4}
    Noise is a stationary or slowly varying process.
\end{assumption}

\begin{assumption}-\label{as:5}
    Noise spectrum does not change drastically during the recording.
\end{assumption}
...
\begin{enumerate}[label=\textbf{\arabic*})]
    ...
    Assuming $d(n)$ and $x(n)$ have no correlation, and we perform a short-time fourier transform on equation \cref{sreduction}:
...
\begin{table}[h]
	...
	\begin{minipage}{0.65\linewidth}
		...
		\label{f0vage_chart}
		\captionof{figure}{Scatter plot of\\fundamental frequency by age\protect\footnote{f0age}}
	\end{minipage}
\end{table}
...
\begin{enumerate}[label=\textbf{\arabic*})]
    ...
    \begin{figure}
        ...
        \begin{subfigure}{.3\textwidth}
          ...
          \caption{$r_t(\tau)$ calculated with \\equation \cref{signal}}
          \label{taped}
        \end{subfigure}%
        \label{YIN}
    \end{figure}
...
\cref{male} and equation \cref{female}.
...
\begin{figure}[h]
	\centering
	\includegraphics[scale=0.35]{Figures/profiles.png}
	\caption{Probe tone rating for F\# and C major}
	\label{probetone}
\end{figure}
\cref{probetone} serves as an example of the profiles, 

The key with the highest correlation will be the estimated key.
