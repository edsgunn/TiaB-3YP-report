...
Assuming $d(n)$ and $x(n)$ have no correlation, and we perform a short-time fourier transform on equation 1:
...
After trying to implement all 3 methods, a major difficulty encountered is that it is hard to set the threshold to 
implement into the method. For example, finding an appropriate $f_c$ that separates desired frequencies from undesired
ones is hard, and $f_c$ depends on the pitch range of the user and the melody he is inputting. 
