...
\begin{figure}
	...
	\begin{tikzpicture}[>=latex', overlay]
		...
		%beige bg
		\node (bg) [right 0.3cm of n1, anchor=center, draw=none, fill=beige, minimum width=4.5cm,minimum height=2cm]{};
		\node (bglabel) [above = 0cm of bg]{Implement noise filter};
...
\begin{enumerate}
	...
	Max pooling is often used in the operation of pooling layer and is the operation used in CREPE. It outputs the maximum value in a patch extracted from the input tensor and discards the non-maximum values. 
...
On top of the convolutional neural network model, we can i
