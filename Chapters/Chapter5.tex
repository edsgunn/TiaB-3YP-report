...
is as below:
\[f_{c,male}(n) = 0.07n^2 - 7.5n + 280, for  4 \leq n\leq 93 \label{male} \] 
\[f_{c,female}(n) = 0.02n^2 - 3n + 287, for  4 \leq n \leq 93 \label{female} \] 
...
%-----------------------------------
\subsection{Possible Models}
There are 4 approaches to detect $f_0$, which again can be classified into time domain and frequency domain.
...
\begin{enumerate}
	...
	Lastly we compute the product of spectrum at harmonics of a frequency and $f_0$ is estimated by:
...
Continuing from the noise filtered signal, we use \emph{[y, sr]= scipy.io.wavfile.read(filename, mmap=False)} function to read the input and feed $y$ and $sr$ 
...
From a musical theory point of view, the most intuitive method to know the key of a piece is to look at the key signature which contains the number of sharps/ flats
